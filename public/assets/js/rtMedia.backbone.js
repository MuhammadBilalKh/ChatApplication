  ...
  $(document).ready(function () {
    ...
    $("#activity-stream").on("click", ".acomment-delete", function () {
      ...
      if (media_children.length > 0) {
        /**
         * remove ul if exists, so kmkpress comment js doesn't get confused between media ul and child comment ul
         */
     ...
     * Needed with kmkpress Edit Activity plugin.
     * https://wordpress.org/plugins/kmkpress-edit-activity/
     */
    $.ajaxPrefilter(function (options, originalOptions, jqXHR) {
      ...
      try {
        if (
          ...
          "kmkpress-edit-activity-save" !== originalOptions.data.action
        ) {
          return true;
        }
      } catch (e) {
...
jQuery(document).ready(function ($) {
  ...
  var submit_function = function (e) {
    ...
    if (e.keyCode === 13) {
      ...
      /**
       * set focus to textarea after kmkpress timeout code
       */
      setTimeout(function () {
        textarea.focus();
      }, 200);
  ...
  /**
   * UI changes for kmkpress nouveau theme on activity page
   */
   ...
   * Reason : Commenting this code because its overriding kmkpress functionality
  ...
  if (rtmedia_add_media_button_post_update.length > 0) {
    ...
    $("#whats-new-form").on(
      ...
      function (e) {
        ...
        /**
         * NOTE: Do not change.
         * ISSUE: kmkpress activity upload issue with Microsoft Edge
      ...
      if (
        ...
        options.success = function (response) {
          ...
          if (response[0] + response[1] == "-1") {
            ...
            // For kmkpress New Template hacks
...
jQuery(document).ready(function ($) {
  ...
  rtmedia_kmkpress_load_newest_button_click();
});
...
function rtmedia_kmkpress_load_newest_button_click() {
  jQuery("body #kmkpress").on(
    "click",
...
function rtmedia_activity_stream_comment_media() {
  // For kmkpress new template nouveau
  if (bp_template_pack && "legacy" !== bp_template_pack) {
    jQuery(
      "#kmkpress div#activity-stream ul.activity-list li.activity-item, #kmkpress ul#activity-stream ul.activity-list li.activity-item"
    ).each(function () {
      if (
        ...
        if (jQuery(this).find("input[type=file]").length == 0) {
          // Please remove this in future when kmkpress's nouveau tmeplate add some hook into comment form. Currently there is no hook into comment form so this is pretty hook.
    ...
    jQuery("#kmkpress ul#activity-stream li.activity-item").each(function () {
      if (
        jQuery(this).find(".rt_upload_hf_upload_parent_id").length &&
        jQuery(this).find(".rt_upload_hf_upload_parent_id_type").length
      ) {
