...
@section('dashboard-content')
    ...
    <div class="kmk-post-container blog-layout-grid masonry grid-columns-2">
        ...
        @forelse ($reviewPendingBlogs as $key => $value)
            @php
                $media = $value->getBlog->first();
                $imagePath = $media?->file_path ? asset('storage/' . $media->file_path) : asset('default.jpg');
            @endphp

            <article id="post-{{ $value->user_blog_id }}"
                    ...
                    <div class="entry-thumbnail">
                        <div class="post-medias">
                            <div class="item-media">
                                <a href="#" style="background-image: url('{{ $imagePath }}');"></a>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="entry-content">
                        ...
                        <div class="entry-meta">
                            <span class="link date-links">
                                <span>{{ $value->published_at }} ({{ ucfirst($value->status) }})</span>
                            </span>
                        </div>
                        ...
                        <div class="entry-excerpt">
                            <p>{!! \Illuminate\Support\Str::limit(strip_tags($value->content), 50) !!}...</p>
                        </div>

                        <div class="read-more">
                            <a href="{{ route('blogs.view', ['id' => $value->user_blog_id]) }}" target="_blank"
                                class="text-info">View Blog</a>
                        </div>
    ...
    <div class="container">
        {{ $reviewPendingBlogs->appends(['type' => 'review-pending'])->links() }}
    </div>
