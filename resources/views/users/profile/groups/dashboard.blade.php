...
@section('group-content')
    ...
    <div id="bp-nouveau-activity-form" class="activity-update-form">
        <form name="whats-new-form" action="{{ route('groups.create_post', ['group' => $groupData->group_id]) }}"
            method="{{ FORM_METHOD_POST }}" id="whats-new-form" class="activity-form activity-form-expanded" enctype="multipart/form-data">
            @csrf

            <div id="whats-new-avatar">

                <a href="https://www.clientbetalink.xyz/MIGVELv1/members-2/wpdeveloper/">
                    <img src="{{ asset($groupData->groupCreatedBy->profile_picture) }}"
                        class="avatar user-1-avatar avatar-50 photo" width="50" height="50"
                        alt="Profile photo of {{ $groupData->groupCreatedBy->username }}">
                </a>

            </div>
            <div id="whats-new-content">
                <div id="whats-new-textarea" style="position: relative;">
                    <textarea name="description" cols="50" rows="4" placeholder="What's new, wpdeveloper?"
                                        ...
                                        <div class="rtm-upload-button-wrapper">
                                            <div id="rtmedia-whts-new-upload-container" style="position: relative;">
                                                <div id="html5_1j8u2f1q1aon1eei190eik911tu3_container"
                                                    ...
                                                    <input id="html5_1j8u2f1q1aon1eei190eik911tu3" type="file"
                                                        name="media[]"
    ...
    <script>
        ...
        jQuery(document).ready(function() {
            ...
            function loadPosts() {
                ...
                fetch(`{{ route('group_posts.load', ['group' => $groupData->group_id]) }}?page=${page}`)
                    .then(res => res.json())
                    .then(data => {
                        if (data.html.trim() !== '') {
        ...
        jQuery(document).ready(function(jQuery) {
            ...
            jQuery(document).on('submit', '.add-comment-form', function(e) {
                ...
                const postId = $form.find('input[name="group_post_id"]').val();

                $btn.prop('disabled', true).val('Posting...');

                jQuery.ajax({
                    url: "{{ route('group_comments.store', ['group' => $groupData->group_id]) }}",
                    method: "{{ FORM_METHOD_POST }}",
                    data: {
                        group_post_id: postId,
            ...
            jQuery(document).on('submit', '.edit-comment-form', function(e) {
                ...
                jQuery.ajax({
                    url: "{{ route('group_comments.update', ['group' => $groupData->group_id]) }}",
                    method: '{{ FORM_METHOD_POST }}',
                    data: {
                        content: content,
                        comment_id: commentId,
