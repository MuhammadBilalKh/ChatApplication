<li class="comment-item comment-container" id="comment-{{ $comment->group_post_comment_id }}" data-comment-id="{{ $comment->group_post_comment_id }}"
    ...
    <div class="acomment-content" id="comment-content-{{ $comment->group_post_comment_id }}">
    ...
    @if ($comment->commentPostedBy->user_id == Auth::user()->user_id)
        <form
            action="{{ route('group_comments.update', [
                'group' => $groupData->group_id,
                'comment' => $comment->group_post_comment_id,
            ]) }}"
            method="{{ FORM_METHOD_POST }}" class="edit-comment-form" id="edit-form-{{ $comment->group_post_comment_id }}"
            style="display: none;" data-comment-id="{{ $comment->group_post_comment_id }}">
            ...
            <div class="ac-reply-content">
                <div class="ac-textarea">
                    <input type="hidden" name="comment_id" value="{{ $comment->group_post_comment_id }}" />
                    <input class="ac-input form-control bp-suggestions edit-comment-text"
                        value="{{ $comment->comment_text }}" name="content" spellcheck="false" required />
                </div>
    ...
    <div class="activity-meta action">
        <div class="generic-button">
            <a class="acomment-reply bp-primary-action show-reply-form-btn" href="#"
                data-comment-id="{{ $comment->group_post_comment_id }}">
                Reply
            </a>
        </div>
    ...
    <form action="{{ route('group_comments.store', ['group' => $groupData->group_id]) }}"
        method="{{ FORM_METHOD_POST }}" class="add-reply-form" style="display: none;">
        @csrf
        <input type="hidden" name="group_post_id" value="{{ $comment->post_id }}" />
        <input type="hidden" name="parent_comment_id" value="{{ $comment->group_post_comment_id }}" />
        ...
        <div class="ac-reply-content">
            <div class="ac-textarea">
                <input type="text" class="ac-input bp-suggestions" name="content" placeholder="Write a reply..."
    ...
    @if ($comment->replies && $comment->replies->count() > 0)
        <ul class="reply-list">
            @foreach ($comment->replies as $reply)
                @include('users.profile.groups.comment_item', [
                    'comment' => $reply,
                    'depth' => $depth + 1,
                    'groupData' => $groupData,
                ])
