    ...
    <div id="group-invites-container">
        ...
        <div class="group-invites-column">
            ...
            <div class="bp-invites-feedback">
                <div class="bp-invites-feedback">
                    <div class="bp-feedback help">
                        <span class="bp-icon" aria-hidden="true"></span>
                        <p>Select members to invite by clicking the + button. Once you've made your selection, use the "Send
                            Invites" navigation item to continue.</p>
                    </div>
                </div>
            </div>
                ...
                <ul id="members-list" class="item-list bp-list">
                    @forelse($membersToInvite as $key => $value)
                        <li class="">
                            ...
                            <div class="action action-{{ $value->user_id }}">

                                <button type="button"
                                    class="button @if(isset($value->getInvitations[$key]) && $value->getInvitations[$key]->status == "pending") btn-delete-invite @else invite-button @endif group-add-remove-invite-button bp-tooltip bp-icons"
                                    data-user-id="{{ $value->user_id }}" data-bp-tooltip="Invite">
                                    <span>
                                         @if(isset($value->getInvitations[$key]) && $value->getInvitations[$key]->status == "pending") Delete Invitation @else Invite @endif
                                    </span>
                                </button>
                            </div>
