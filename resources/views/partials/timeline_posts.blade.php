{{--
    This partial assumes it is included *within* a loop that defines $post,
    or that $post is passed as a parameter to this partial.
    If not, wrap this in a loop or pass $post when including this file.
--}}
...
@foreach ($posts as $post)
    ...
    <li class="activity-item animate-item slideInUp{{ $topLevelCommentsCount ? ' has-comments' : '' }}"
        data-id="{{ $post->post_id }}" style="animation-name: slideInUp;">
        {{-- USER AVATAR --}}
        <div class="activity-avatar item-avatar">
            <a href="#">
                <img src="{{ asset($post->postUploadedBy->profile_picture ?? 'assets/images/default.png') }}"
                    width="50" height="50"
                    class="avatar user-{{ $post->postUploadedBy->user_id }}-avatar avatar-200 photo"
        ...
        <div class="activity-content">
            ...
            {{-- POST ACTION BUTTONS --}}
            <div class="activity-meta action flex gap-2">
        ...
        <div class="activity-comments post-comments" id="comments-{{ $post->post_id }}" style="display:none;">
            <ul class="comment-list">
                @foreach ($topLevelComments as $comment)
                    @include('partials.modal_post_comment', ['comment' => $comment, 'depth' => 0])
                @endforeach
            </ul>
